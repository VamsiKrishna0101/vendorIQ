...
function ConfidenceRing({ score }: { score: number }) {
  ...
  return (
    <svg width="140" height="140" viewBox="0 0 140 140">
      ...
      <text x="70" y="82" textAnchor="middle" fill="#6B6B72" fontSize="9" letterSpacing="2">SCORE</text>
    </svg>
  );
}

function RadarChart({ evaluations }: { evaluations: any[] }) {
  const sz = 200, cx = 100, cy = 100, maxR = 75;
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <svg width={sz} height={sz} viewBox={`0 0 ${sz} ${sz}`}>
        {grid.map((g,i) => <polygon key={i} points={g} fill="none" stroke="#1E1E26" strokeWidth="1"/>)}
        {axes.map(ax => {
          const ep = pt(ax.a, 100);
          return <line key={ax.key} x1={cx} y1={cy} x2={ep.x} y2={ep.y} stroke="#2A2A35" strokeWidth="1"/>;
        })}
        {evaluations.slice(0,5).map((v,i) => (
          <polygon key={v.vendor} points={poly(v.fit_analysis)} fill={COLORS[i]+'18'} stroke={COLORS[i]} strokeWidth="1.5" opacity={0.9}/>
        ))}
        {axes.map(ax => {
          const lp = pt(ax.a, 115);
          return <text key={ax.key} x={lp.x} y={lp.y} textAnchor="middle" fill="#6B6B72" fontSize="8" letterSpacing="1">{ax.label.toUpperCase()}</text>;
        })}
      </svg>
      <div className="flex flex-wrap gap-3 justify-center">
        {evaluations.slice(0,5).map((v,i) => (
          <div key={v.vendor} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-[#6B6B72]">{v.vendor}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="relative overflow-hidden border-b border-[#1E1E22]"
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-10 py-16 flex flex-col lg:flex-row items-center gap-12">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-5">
              ...
              <span className="text-[9px] font-bold tracking-[0.35em] text-[#E8A930] uppercase">Decision Intelligence Report · VendorIQ</span>
            </div>
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[11px] text-[#4B4B60]">Rank #1 · {evals.length} vendors evaluated</span>
            </div>
            ...
            <p className="text-[14px] text-[#7A7A86] leading-relaxed max-w-xl">{verdict.executive_summary}</p>
          </div>
          <div className="flex flex-col items-center gap-3 flex-shrink-0">
            <ConfidenceRing score={winner?.score??0}/>
            <div className="text-center">
              <div className="text-[9px] tracking-[0.25em] text-[#4B4B60] uppercase">Debate Quality</div>
        ...
        <div className="relative z-10 border-t border-[#E8A930]/10 bg-[#E8A930]/[0.02] px-10 py-4">
          <div className="max-w-7xl mx-auto flex items-center gap-4">
            <span className="text-[9px] font-bold text-[#E8A930] tracking-[0.25em] uppercase whitespace-nowrap">Board Directive</span>
            <div className="h-px flex-1 bg-[#1E1E22]"/>
            <span className="text-[13px] text-[#D0D0D8] italic">"{verdict.strategic_recommendation}"</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#0D0D0F] border-b border-[#1E1E22]">
        <div className="max-w-7xl mx-auto px-10 py-4 flex flex-wrap gap-10">
          {[
            ...
            {label:'Most Influential', value:verdict.most_influential_agent||'—'},
          ].map(m=>(
            <div key={m.label}>
              <div className="text-[9px] text-[#4B4B60] uppercase tracking-widest mb-0.5">{m.label}</div>
              <div className="text-[16px] font-bold text-[#E8E8F0]">{m.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-10 pb-20">
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            <div className="flex items-center gap-3 mb-5">
              <h2 className="text-[10px] font-bold text-[#4B4B60] uppercase tracking-widest">Executive Ranking Matrix</h2>
              <div className="h-px flex-1 bg-[#1E1E22]"/>
            </div>
                ...
                <thead>
                  <tr className="bg-[#0D0D0F] border-b border-[#1E1E22]">
                    {['#','Vendor','Score','Tech','Fin','Gov','Status'].map(h=>(
                      <th key={h} className="text-left px-4 py-3 text-[9px] font-bold text-[#4B4B60] uppercase tracking-widest">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {evals.map((v:any,i:number)=>{
                    ...
                    return (
                      <tr key={v.vendor} className={`border-b border-[#1E1E22] ${i===0?'bg-[#E8A930]/[0.04]':'bg-[#09090B]'} hover:bg-[#111115] transition-colors`}>
                        ...
                        <td className="px-4 py-3 text-[#6B6B72]">{v.fit_analysis?.technical??'—'}</td>
                        <td className="px-4 py-3 text-[#6B6B72]">{v.fit_analysis?.financial??'—'}</td>
                        <td className="px-4 py-3 text-[#6B6B72]">{v.fit_analysis?.governance??'—'}</td>
                        <td className="px-4 py-3">
          ...
          <div className="bg-[#0D0D0F] border border-[#1E1E22] rounded-2xl p-6 flex flex-col items-center justify-center">
            <h3 className="text-[9px] font-bold text-[#4B4B60] uppercase tracking-widest mb-6 text-center">Multi-Dimensional Analysis</h3>
        ...
        <div className="mt-14">
          <div className="flex items-center gap-3 mb-8">
            <h2 className="text-[10px] font-bold text-[#4B4B60] uppercase tracking-widest">Vendor Intelligence Briefs</h2>
            <div className="h-px flex-1 bg-[#1E1E22]"/>
          </div>
                ...
                <motion.div key={v.vendor} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:0.1*i}}
                  ...
                  <div className={`px-8 py-5 border-b border-[#1E1E22] flex items-center gap-5 ${isTop?'bg-[#E8A930]/[0.03]':''}`}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-[11px] text-[#4B4B60] mt-0.5">Decision Score: <b className="text-[#E8E8F0]">{v.score}/100</b></div>
                    </div>
                    <div className="grid grid-cols-3 gap-4 text-center">
                      {[['Technical',v.fit_analysis?.technical],['Financial',v.fit_analysis?.financial],['Governance',v.fit_analysis?.governance]].map(([k,val])=>(
                        <div key={String(k)}>
                          ...
                          <div className="text-[8px] text-[#4B4B60] uppercase tracking-wider">{k}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="px-8 py-6 grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-1">
                      <h4 className="text-[9px] font-bold text-[#4B4B60] uppercase tracking-widest mb-3">Strategic Rationale</h4>
                      <p className="text-[12px] text-[#A0A0A8] leading-relaxed">{v.justification}</p>
                    </div>
                    <div>
                      <h4 className="text-[9px] font-bold text-[#16A34A] uppercase tracking-widest mb-3">Advantages</h4>
                      <ul className="space-y-2">
                        {v.pros?.slice(0,4).map((p:string,pi:number)=>(
                          <li key={pi} className="text-[11px] text-[#C0C0C8] flex items-start gap-2">
                    ...
                    <div>
                      <h4 className="text-[9px] font-bold text-[#EF4444] uppercase tracking-widest mb-3">Risk Factors</h4>
                      <ul className="space-y-2">
                        {v.cons?.slice(0,4).map((c:string,ci:number)=>(
                          <li key={ci} className="text-[11px] text-[#C0C0C8] flex items-start gap-2">
        ...
        <div className="mt-14">
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-[10px] font-bold text-[#4B4B60] uppercase tracking-widest">Enterprise Risk Register</h2>
            <div className="h-px flex-1 bg-[#1E1E22]"/>
            <span className="text-[9px] text-[#4B4B60]">{risks.length} signals detected</span>
          </div>
          <div className="rounded-2xl border border-[#1E1E22] overflow-hidden">
            <table className="w-full text-[12px]">
              <thead>
                <tr className="bg-[#0D0D0F] border-b border-[#1E1E22]">
                  {['Severity','Risk','Mitigation','Raised By'].map(h=>(
                    <th key={h} className="text-left px-5 py-3 text-[9px] font-bold text-[#4B4B60] uppercase tracking-widest">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {risks.map((r:any,i:number)=>(
                  <tr key={i} className="border-b border-[#1E1E22] bg-[#09090B] hover:bg-[#0F0F13] transition-colors">
                    <td className="px-5 py-4">
                      <span className={`text-[9px] font-bold px-2 py-0.5 rounded ${SEV_CLS[r.severity]||SEV_CLS.LOW}`}>{r.severity}</span>
                    </td>
                    <td className="px-5 py-4 text-[#C0C0C8] max-w-xs">{r.risk}</td>
                    <td className="px-5 py-4 text-[#6B6B72] max-w-xs">{r.mitigation}</td>
                    <td className="px-5 py-4 text-[#4B4B60] text-[11px]">{r.raised_by}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="mt-14 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div>
            <div className="flex items-center gap-3 mb-5">
              <h2 className="text-[10px] font-bold text-[#4B4B60] uppercase tracking-widest">Conditions Before Signing</h2>
              <div className="h-px flex-1 bg-[#1E1E22]"/>
            </div>
            <div className="flex flex-col gap-3">
              {verdict.conditions_before_signing?.map((c:string,i:number)=>(
                <div key={i} className="flex items-start gap-4 bg-[#0D0D0F] border border-[#1E1E22] rounded-xl px-5 py-4">
                  ...
                  <p className="text-[12px] text-[#A0A0A8] leading-relaxed">{c}</p>
                </div>
              ))}
            </div>
          </div>
          <div>
            <div className="flex items-center gap-3 mb-5">
              <h2 className="text-[10px] font-bold text-[#4B4B60] uppercase tracking-widest">Strategic Dissent</h2>
              <div className="h-px flex-1 bg-[#1E1E22]"/>
            </div>
            <div className="bg-[#0D0D0F] border border-[#E8A930]/20 rounded-xl p-6">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-[9px] font-bold text-[#E8A930] tracking-widest uppercase">⚠ Minority Position</span>
                <span className="text-[9px] text-[#4B4B60]">— {verdict.minority_dissent?.agent}</span>
              </div>
              <p className="text-[12px] text-[#C0C0C8] italic mb-4 leading-relaxed">"{verdict.minority_dissent?.argument}"</p>
              <div className="border-t border-[#1E1E22] pt-4">
                <div className="text-[9px] text-[#4B4B60] uppercase tracking-widest mb-1">Overruled Because</div>
                <p className="text-[11px] text-[#6B6B72] leading-relaxed">{verdict.minority_dissent?.mitigation_plan}</p>
              </div>
            </div>
            {verdict.governance_flags?.length > 0 && (
              <div className="mt-4 bg-[#0D0D0F] border border-[#1E1E22] rounded-xl p-5">
                <div className="text-[9px] font-bold text-[#4B4B60] uppercase tracking-widest mb-3">Governance Flags</div>
                {verdict.governance_flags.map((f:string,i:number)=>(
                  <div key={i} className="flex items-start gap-2 mb-2">
                    <span className="text-[#E8A930] text-[10px] flex-shrink-0">⚑</span>
                    <p className="text-[11px] text-[#6B6B72]">{f}</p>
                  </div>
                ))}
              </div>
            )}
        ...
        <div className="mt-16 pt-8 border-t border-[#1E1E22] flex items-center justify-between">
          <div>
            ...
            <div className="text-[9px] text-[#4B4B60] mt-0.5">AI-powered procurement intelligence · {dateStr}</div>
          </div>
