"""
customer_node.py — Runs alongside debate rounds (1 and 2)

Generic stateless node for ALL customer panel members.
"""
...
from Agents.customer_agents import CustomerAgent
...
def _get_agent(model_name: str = "gemini-2.5-flash-lite") -> CustomerAgent:
    gemini = GeminiService(model_name=model_name)
    return CustomerAgent(gemini_service=gemini)


def make_customer_node(agent_id: str, round_number: int):
    async def node(state: DebateState) -> dict:
        session_id = state.get("session_id", "unknown")
        
        if not state.get("intelligence"):
            error_msg = f"customer_node [{agent_id}, round {round_number}]: intelligence not found"
    ...
    node.__name__ = f"customer_node_{agent_id}_r{round_number}"
    return node
